...
class Encoder(nn.Module):
    def __init__(self, d_model, head_num, dropout):
        ...
        self.embedding = Embedding(5001, 36, 2000)
...
class Transformer(nn.Module):
    def __init__(self, d_model, head_num, dropout):
        ...
        self.source_pad_idx = 5000
        self.target_pad_idx = 5000
        self.encoder = Encoder(d_model, head_num, dropout)
        self.decoder = Decoder(d_model, head_num, dropout)

    def forward(self, enc, dec):
        src_mask = self.make_source_mask(enc)
        tgt_mask = self.make_target_mask(dec)
        x = self.encoder(enc, src_mask)
        x = self.decoder(dec, x, src_mask, tgt_mask)
        return x

    def make_source_mask(self, x):
        src_mask = (x == self.source_pad_idx).unsqueeze(1).unsqueeze(1)
        print("src_mask shape: ", src_mask.shape)
        print(src_mask)
        return src_mask

    def make_target_mask(self, x):
        padding_mask = (x == self.target_pad_idx).unsqueeze(1).unsqueeze(1)
        seq_num, seq_len = x.size()
        look_ahead_mask = torch.tril(torch.ones(seq_len, seq_len)) == 0
        look_ahead_mask = look_ahead_mask.unsqueeze(0).unsqueeze(0)
        tgt_mask = padding_mask | look_ahead_mask
